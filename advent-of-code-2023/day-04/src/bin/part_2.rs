...
fn process(input: &str) -> u32 {
    ...
    let matches_counts = cards
        .iter()
        .map(|card| card.winning_numbers.intersection(&card.my_numbers).count() as u32)
        .collect::<Vec<u32>>();

    let card_count = (0..matches_counts.len())
        .map(|i| (i, 1))
    ...
    matches_counts
        .iter()
        .enumerate()
        .fold(card_count, |mut acc, (index, card_count)| {
            for i in (index + 1)..(index + 1 + *card_count as usize) {
                let additional_card_count = *acc.get(&index).unwrap();
                acc.entry(i).and_modify(|value| {
                    *value += additional_card_count;
                });
            }
            acc
        })
