...
fn get_all_file_contents_in_folder(folder: &str, file_type: &str) -> Result<Vec<Frontmatter>, Error> {
    ...
    for toml_file in post_folder_toml_files {
        ...
        if toml_file.path().is_file() {
            ...
            let frontmatter: Frontmatter = toml::from_str(&fm_content).unwrap(); //cannot convert toml::de::Error to std::io::Error so unwrapping which is danger
...
pub async fn index(templates: web::Data<tera::Tera>) -> impl Responder {
    ...
    HttpResponse::InternalServerError().content_type(CONTENT_TYPE_TEXT_HTML).body("<p>Something went wrong</p>")
}